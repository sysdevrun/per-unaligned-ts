 ...
 *
 * This checks for the presence of the level 2 signature, public key,
 * and signing algorithm OID — the three fields required for self-contained
 * Level 2 verification.
 *
 * @param bytes - The raw barcode payload bytes.
 * @returns True if the barcode contains a Level 2 signature that can be verified.
 */
export function hasLevel2Signature(bytes: Uint8Array): boolean {
  try {
    const data = extractSignedData(bytes);
    return !!(data.level2Signature && data.level2PublicKey && data.level2SigningAlg);
  } catch {
    return false;
  }
}
 ...
 *
 * @param bytes - The raw barcode payload bytes.
 * @param publicKey - The Level 1 public key (raw SPKI DER bytes or KeyObject).
 * @returns Verification result with valid flag and optional error.
 */
